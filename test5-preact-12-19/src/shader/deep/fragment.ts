...
void main(){
    ...
    //7利用通过取模达到反复效果,百叶窗,白黑渐变
    // float strength = mod(vUv.y * 10.0 , 1.0) ;
    // gl_FragColor =vec4(strength,strength,strength,1);


    //8利用step(edge, x)如果x < edge,返回0.0,否则返回1.0,黑白条纹
    // float strength =  mod(vUv.y * 10.0 , 1.0) ;
    ...
    // 11条纹相加,黑方格白背景,mod(x,y)取余x除y的余数
    // float strength = step(0.8, mod(vUv.x * 10.0 , 1.0));
    // strength += step(0.8, mod(vUv.y * 10.0 , 1.0));
    // gl_FragColor =vec4(strength,strength,strength,1);


    // 12条纹相乘,黑色背景整齐排列的白方格,mod(x,y)取余x除y的余数
    // float strength = step(0.8, mod(vUv.x * 10.0 , 1.0));
    // strength *= step(0.8, mod(vUv.y * 10.0 , 1.0));
    // gl_FragColor =vec4(strength,strength,strength,1);

    // 12条纹相减,黑背景,白条,mod(x,y)取余x除y的余数
    // float strength = step(0.8, mod(vUv.x * 10.0 , 1.0));
    // strength -= step(0.8, mod(vUv.y * 10.0 , 1.0));
    // gl_FragColor =vec4(strength,strength,strength,1);


    // 13方块图形,白方块,黑背景,mod(x,y)取余x除y的余数
    ...
    // 21 利用取整,实现条纹渐变,floor(x)向下取整,水平黑向白渐变条纹
    ...
    // vec2 rotateUv = rotate(vUv,-uTime*500.0,vec2(0.5));
    // float alpha =  1.0 - step(0.5,distance(vUv,vec2(0.5)));
