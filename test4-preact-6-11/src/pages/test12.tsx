...
export default function Test12() {
  ...
  function animate() {
    ...
    for (let i = 0; i < points.length; ++i) {
      if (points[i].position.y < -100) {
        points[i].position.y = 100;
      } else {
        points[i].position.y -= 0.06;
      }
    }
    for (let i = 0; i < points2.length; ++i) {
      if (points2[i].position.y < -100) {
        points2[i].position.y = 100;
      } else {
        points2[i].position.y -= 0.06;
      }
    }
  ...
  useEffect(() => {
    if (test12.current) {
      test12.current.appendChild(stats.dom);
