...
export default function Test5() {
  ...
  controls.enableDamping = true; // 是否惯性
  controls.enableZoom = false; // 取消默认的缩放
  controls.enablePan = true; // 开启右键拖动
  controls.minDistance = 100;
  controls.maxDistance = 850;
  const gui = new GUI();

  const city = new City(scene, camera, 5, 0, controls);
  city.loadCity('../../public/model/beijing.fbx');
  ...
  gui.add({
    snow: () => {
      if (city.snow.length == 2) {
        city.stopSnow = !city.stopSnow;
        if (city.stopSnow) {
          city.snow[0].visible = false;
          city.snow[1].visible = false;
        } else {
          city.snow[0].visible = true;
          city.snow[1].visible = true;
        }
      }
    }
  }, 'snow');
  gui.add({
    rain: () => {
      if (city.rain.length == 2) {
        city.stopRain = !city.stopRain;
        if (city.stopRain) {
          city.rain[0].visible = false;
          city.rain[1].visible = false;
        } else {
          city.rain[0].visible = true;
          city.rain[1].visible = true;
        }
      }
    }
  }, 'rain');
  gui.add({
    fog_warning: () => {
      city.stopSmoke = !city.stopSmoke;
      if (city.stopSmoke) {
        if (city.smoke) {
          city.smoke.visible = false;
        }
      } else {
        if (city.smoke) {
          city.smoke.visible = true;
        }
      }
    }
  }, 'fog_warning');
  ...
  function animate() {
    ...
    city.time += 0.02;
    if (city.obj) {
      city.obj.traverse((child) => {
        city.createOutLine(child as THREE.Mesh);
      });
    }
    // 雷达
    for (let i in city.radars) {
      city.createRadar(i);
    }
    // 光墙
    city.createWall();
  ...
  return (<>
    <div ref={test3} onClick={canvasActive}></div>
  </>);
}
